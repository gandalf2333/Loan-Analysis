...
def load_ticker_file():
    ...
    """
    try:
        # Assuming the CSV is comma-separated now based on your confirmation
        df = pd.read_csv("NASDAQ_NYSE_tickers.csv")

        st.sidebar.info(f"Loaded ticker file. DataFrame shape: {df.shape}")
        # st.sidebar.dataframe(df.head()) # Uncomment for full debug of the loaded raw dataframe
        ...
        st.sidebar.error("`NASDAQ_NYSE_tickers.csv` not found at the specified path. Please ensure it's in the same directory as the app.")
        return pd.DataFrame()
    except Exception as e:
        st.sidebar.error(f"Error loading ticker file: {e}. Check file format and content.")
        st.sidebar.error(traceback.format_exc()) # Show full traceback for more details
        return pd.DataFrame()
...
def fetch_financial_data_multi_year(ticker_symbol, frequency='annual'):
    ...
    try:
        # Create a session with a common browser user-agent to avoid common YF blocks
        ...
        stock_info = stock.info # This is where the error occurred in previous versions if user-agent was an issue
        ...
        hist = stock.history(period="4y") # Fetch 4 years of historical price data for plotting
        ...
        st.error(f"This often happens when Yahoo Finance blocks requests from cloud platforms. The custom user-agent trick may not always work. Please try again later.")
...
def calculate_ratios_multi_year(balance_sheet, income_statement, cashflow_statement, stock_info_latest):
    ...
    # Ensure periods are sorted, especially for plotting trends
    if not common_periods.empty:
        ...
        else: # For non-datetime index, try converting if possible or sort as strings
            try:
                common_periods = pd.to_datetime(common_periods, errors='coerce').dropna().sort_values(ascending=True)
            except:
                common_periods = sorted(common_periods) # Fallback to string sort

    for period in common_periods:
        ratios = {}
        # Safely access period data; if conversion to datetime failed, period might be string
        ...
        shareholder_equity = get_safe_value(bs_period, ['Stockholders Equity', 'Total Stockholder Equity', 'StockholdersEquity']) # Added 'StockholdersEquity'
        total_assets = get_safe_value(bs_period, ['Total Assets'])
        long_term_debt = get_safe_value(bs_period, ['Long Term Debt', 'LongTermDebt']) # Added 'LongTermDebt'
        ...
        if total_debt == 0 and pd.notna(extracted_total_debt): # Use extracted if available and valid
        ...
        interest_expense_val = get_safe_value(is_period, ['Interest Expense', 'InterestAndDebtExpense']) # Added 'InterestAndDebtExpense'
        ...
        cap_ex = abs(cap_ex_val) if pd.notna(cap_ex_val) else None # CapEx is usually negative, take absolute for ratio
        ...
        # Format period label for DataFrame column name
    ...
    # Sort columns (periods) in descending order to show most recent first
...
def simulate_loan_impact(latest_financial_data, loan_amount, interest_rate_decimal):
    ...
    # Extract EBIT and current Interest Expense from the latest financial data
    ...
    if pd.notna(ebit) and pd.notna(current_interest_expense):
        ...
        if new_total_interest_expense != 0:
            ...
            # Handle division by zero for interest coverage ratio
            if ebit > 0:
                results['Pro-Forma Interest Coverage Ratio'] = float('inf') # Infinite if EBIT is positive and interest is zero
                results['Acceptable Coverage (>1.25x)?'] = True
            elif ebit < 0:
                results['Pro-Forma Interest Coverage Ratio'] = float('-inf') # Negative infinite if EBIT is negative and interest is zero
                results['Acceptable Coverage (>1.25x)?'] = False
            else: 
                results['Pro-Forma Interest Coverage Ratio'] = 0.0 # If both EBIT and interest are zero
                results['Acceptable Coverage (>1.25x)?'] = False 
    return results
...
def assess_risk_from_ratios(latest_ratios_series):
    ...
    def check_ratio(name, threshold, is_greater_better=True):
        ...
        if pd.notnull(val):
            ...
            # Special formatting for percentages in risk summary
    ...
    for name, thresh in thresholds.items():
        # Debt-to-Equity is lower-is-better, others are generally higher-is-better
...
# Load tickers for auto-completion
ticker_df = load_ticker_file()

# Prepare options for the selectbox
ticker_options_display = [''] # Add an empty option at the start
ticker_symbol_map = {} # To map display label back to ticker
...
selected_ticker_display = st.sidebar.selectbox(
    ...
    index=0, # Start with an empty selection
    help="Start typing a ticker or company name to search. For best results, select from the dropdown suggestions. Requires 'NASDAQ_NYSE_tickers.csv' file in the same directory."
)
...
if selected_ticker_display:
    ...
    if selected_ticker_display in ticker_symbol_map:
        ...
        # If user typed directly and didn't select, try to extract or use as-is
        ...
        if not ticker_df.empty and ticker not in ticker_df['Symbol'].values:
            st.sidebar.warning(f"'{ticker}' may not be a recognized ticker in the loaded list. Please verify the ticker or try selecting from the suggestions.")
elif st.session_state.get('last_successful_ticker'): # Keep previous ticker if selected_ticker_display is empty
...
if analyze_button and ticker:
    st.session_state['last_successful_ticker'] = ticker # Store for re-runs
    ...
    if stock_info and get_safe_value(stock_info, 'shortName'):
        ...
        # Dynamically create tabs based on data availability
        ...
        if "🤖 AI Financial Summary" in tabs_list_names:
            with created_tabs[tab_idx]:
                ...
                **Disclaimer:** This summary is generated by an AI model (Google Gemini) based on the financial data provided. 
                ...
                """)
                if ratios_df is not None and not ratios_df.empty:
                    latest_ratios = ratios_df[ratios_df.columns[0]] # Get the most recent period's ratios
        ...
        with created_tabs[tab_idx]: # Loan Impact & Risk
            ...
            loan_sim_inputs = {}
            # Ensure latest income statement data is available for simulation inputs
            ...
            if ratios_df is not None and not ratios_df.empty:
                latest_ratios_series = ratios_df[ratios_df.columns[0]] # Get the most recent period's ratios
        ...
        st.session_state['last_successful_ticker'] = None # Clear last successful ticker if current attempt fails
